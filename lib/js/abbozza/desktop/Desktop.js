...
Desktop.addFrame = function (frame,id = null) {
...
Desktop.drag = function (event) {
    ...
    if (Desktop.draggedFrame != null) {
        ...
        var top = (pointerPosY <= 0);
        var bottom = (pointerPosY >= Desktop.desktop.offsetHeight-1);
        var left = (pointerPosX <= 0);
        var right = (pointerPosX >= Desktop.desktop.offsetWidth-1);
...
Desktop.dragFrame = function (frame,event) {
    ...
    if ( (event.target != frame.titleLeft) &&  (event.target != frame.titleRight) ) return;
