...
Abbozza.init = function (systemPrefix, devAllow, helpUrl) {
    ...
    // Set help URL
    if (typeof helpUrl != "undefined") this.HELP_URL = helpUrl;
...
Abbozza.setBlocks = function(xml) {
    ...
    if (this.blockMain == null) {
        ...
        if ( this.blockDevices != null ) this.blockDevices.moveBy(20, 20);
    }
 ...
 * @returns {undefined}
 */
Abbozza.openOverlayDialog = function (html, callback) {
    overlay = document.getElementById("overlay");
    overlay_content = document.getElementById("overlay_content");
    overlay.style.display = "block";
    overlay.style.zIndex = 42;
    overlay_content.innerHTML = "<div>" + html + "</div>";
...
Abbozza.setSketchFromPath = function(path) {
  ...
  // console.log("Setting sketch from path: " + path);
  ...
  /* if ( path.startsWith("!") ) {
     sketchPath = path.substring(1);
  } else {
     sketchPath = "/abbozza/load?" + path;
  } */
 
  Connection.getXML(sketchPath,
    function (response) {
      // Check if a sketch is in the session storage
      Abbozza.setSketch(response, document.location.hash.substr(1));
    },
    function (response) {
      var args = [];
