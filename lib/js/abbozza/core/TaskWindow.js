...
TaskWindow.show = function() {
    ...
    TaskWindow.triggerOnView(TaskWindow.page_);
}
...
TaskWindow.triggerOnView = function(page) {
    Page.onShow = null;
    Page.onHide = null;
    ...
    if ( !Task.initialized ) {
        scripts = page.getElementsByClassName("task");
        // Execute only the first
        for (var idx = 0; idx < scripts.length; idx++) {
            if (scripts[idx].tagName == "SCRIPT") {
                var script = document.createElement("script");
                script.textContent = scripts[idx].textContent;
    ...
    scripts = page.getElementsByClassName("page");
    // Execute only the first
    for (var idx = 0; idx < scripts.length; idx++) {
        if (scripts[idx].tagName == "SCRIPT") {
            var script = document.createElement("script");
