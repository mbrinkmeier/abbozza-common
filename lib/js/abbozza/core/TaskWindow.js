...
TaskWindow.init = function () {
    ...
    TaskWindow.frame.div.addEventListener('contextmenu', function (event) {
        Blockly.ContextMenu.show(event, TaskWindow.customContextMenu(), false);
    });

    TaskWindow.frame.div.addEventListener('click', function (event) {
    ...
    TaskWindow.nav_mainSketch = TaskWindow.frame.addTitleButton(
            ...
            _("gui.task_tooltip_main")
            );

    TaskWindow.editButton_ = TaskWindow.frame.addTitleButton(
            "<svg viewBox='0 0 20 20'><path stroke-width='1' stroke='black' fill='none' d='M4,1 l4,0 l0,4 l-3,3 l-4,0 l0,-4 l3,-3 M8,1 l9,9 l2,9 l-9,-2 l-9,-9 M8,5 l9,9 M5,8 l9,9 M17,13 l0,1 l-3,3 l-1,0'/><circle cx='4.5' cy='4.5' r='1.5'/></svg>",
            TaskWindow.editClicked_,
            _("gui.task_tooltip_edit")
            );
    ...
    TaskWindow.setEditable( Abbozza.areTasksEditable() );
...
TaskWindow.setEditorSize = function () {
    if (TaskWindow.ckeditor != null) {
        var height = TaskWindow.page_wrapper_.offsetHeight;
        TaskWindow.ckeditor.resize("100%", "" + height, true);
    }
}
...
TaskWindow.resize = function (event) {
    ...
    if (TaskWindow.ckeditor != null) {
        TaskWindow.ckeditor.resize("100%",""+h, true);
    }
}
...
TaskWindow.setPage_ = function (page, storePage) {
    ...
    if ((page >= TaskWindow.pages_.length) || (page < 0))
        return;
...
TaskWindow.setPosition = function (x, y) {
    TaskWindow.frame.setPosition(x, y);
};
...
TaskWindow.setEditable = function (editable) {
    if (TaskWindow.editButton_) {
        if (editable) {
            TaskWindow.editButton_.style.display = "block";
        } else {
            if ( TaskWindow.editing_ ) {
                TaskWindow.showContent();
            }
...
/**
 * Create the context menu for the TaskWindow.
 * 
 * @returns {Array|TaskWindow.customContextMenu.options}
 */
TaskWindow.customContextMenu = function () {
    var options = [];

    if ( Abbozza.areTasksEditable() ) {
        if ( !TaskWindow.editing_) {
            var editOption = {
                text: _("gui.task_tooltip_edit"),
                enabled: true,
                callback: function (e) {
                    if (TaskWindow.editing_ == false) {
                        TaskWindow.editClicked_(e);
                    }
                }
            };
            ...
            var endEditOption = {
                text: _("gui.task_tooltip_end_edit"),
                enabled: true,
                callback: function (e) {
                    if (TaskWindow.editing_ == true) {
                        TaskWindow.editClicked_(e);
            ...
            options.push(endEditOption);
        }
    }

    var reloadOption = {
        text: _("gui.task_tooltip_reload"),
        enabled: true,
        callback: TaskWindow.reload_
    };
    options.push(reloadOption);

    var mainOption = {
        text: _("gui.task_tooltip_main"),
        enabled: true,
        callback: TaskWindow.mainSketch_
    };
