...
TaskWindow.updateNav_ = function() {
    ...
    //   // TaskWindow.nav_.prev_.style.display = "none";
    //   // TaskWindow.nav_.next_.style.display = "none";
    //   // TaskWindow.nav_.pageno_.style.display = "none";
    //   TaskWindow.nav_.prev_.className = "taskOverlayNavButtonDisabled";
    //   TaskWindow.nav_.next_.className = "taskOverlayNavButtonDisabled";
    // }
    
    // if ( TaskWindow.nav_.style.display == "none" ) {
    //   TaskWindow.page_.style.marginBottom = "0px";
    // } else {
...
TaskWindow.triggerOnView = function() {
    TaskWindow.triggerOnView_(TaskWindow.page_.firstChild);
}

TaskWindow.triggerOnView_ = function(node) {
    while ( node != null ) {
        if ( node.nodeType == 1) {
            if ( node.hasAttribute("onview") ) {
                eval(node.getAttribute("onview"));
            }
            if ( node.firstChild ) {
                TaskWindow.triggerOnView_(node.firstChild);
            }
        }
        node = node.nextSibling;
    }
