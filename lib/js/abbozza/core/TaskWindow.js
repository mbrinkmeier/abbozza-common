...
TaskWindow.init = function () {
    ...
    if ( TaskWindow.closable ) {
      ...
      // TaskWindow.closeButton_.style.color = 'white';
      // TaskWindow.closeButton_.style.fontSize = '24px';
      TaskWindow.closeButton_.innerHTML = '<img src="img/nav/close.png"/>';
      // TaskWindow.closeButton_.innerHTML = '&#x2612;';
    ...
    if ( TaskWindow.editable ) {
      ...
      // TaskWindow.editButton_.style.color = 'white';
      // TaskWindow.editButton_.style.fontSize = '24px';
      // TaskWindow.editButton_.innerHTML = '&#x270E;';
    ...
    // TaskWindow.nav_.next_.innerHTML = "&#x23F5;";
    ...
    // TaskWindow.nav_.nextSketch_.innerHTML = "&#x23ED;";
    ...
    // TaskWindow.nav_.mainSketch_.innerHTML = "&#x23CF;";  // &#x2302
    ...
    // TaskWindow.nav_.zoomOut_.innerHTML = "&ominus;";  // &#x2302
    ...
    // TaskWindow.nav_.zoomIn_.innerHTML = "&oplus;";  // &#x2302
...
TaskWindow.resetContent = function() {
    TaskWindow.mainSketch = null;
    TaskWindow.prevSketch = null;
    TaskWindow.nextSketch = null;
    TaskWindow.setContent("",true);
    TaskWindow.taskScript_ = null;
    Task = {};
...
TaskWindow.setPage_ = function ( page, storePage ) {
    ...
    if ( TaskWindow.editing_ ) {
      if ( TaskWindow.ckeditor != null ) {
        ...
        TaskWindow.triggerOnHide(TaskWindow.pages_[TaskWindow.currentPage_]);
        TaskWindow.ckeditor.destroy();
        TaskWindow.ckeditor = null;
      }
      ...
      TaskWindow.nav_.pageno_.textContent = (TaskWindow.currentPage_ + 1) + " / " + TaskWindow.pages_.length;
      TaskWindow.readPageScript(TaskWindow.page_);
      TaskWindow.executePageScript(TaskWindow.page_);
      TaskWindow.triggerOnShow(TaskWindow.page_);
      TaskWindow.triggerOnLoad();
...
TaskWindow.readPageScript = function(page) {
    var scripts = page.getElementsByTagName("PAGESCRIPT");
    if ( scripts[0] ) {
        TaskWindow.pageScript_ = scripts[0].textContent;
    } else {
        TaskWindow.pageScript_ = null;
    }   
}


TaskWindow.executePageScript = function(page) {
    Page = {};
    if ( TaskWindow.pageScript_ ) {
...
TaskWindow.executeTaskScript = function(page) {
    Task = {};
...
TaskWindow.triggerOnShow = function(page) {
    if ( !TaskWindow.isVisible()) return;
    ...
    if ( (Task === undefined) || (Task.initialized === undefined) || ( Task.initialized == false) ) {
        Task = {};
        Task.initialized = false;
        TaskWindow.executeTaskScript(page);
        if ( Task.onShow != null ) {
            Task.onShow();
        }
    ...
    TaskWindow.executePageScript(page);  
...
TaskWindow.triggerOnHide = function(page) {
    if ( Page.onHide != null ) {
        Page.onHide();
    }
}
...
TaskWindow.triggerOnLoad = function() {
    console.log("Call to deprectaed TaskWindow.triggeroOnLoad()");
