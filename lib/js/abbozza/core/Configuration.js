...
Configuration._apply = function (config, init) {
    ...
    // Get editing mode
    var editMode = Configuration.getParameter("mode");
    if ( editMode ) {
        if ( Number(editMode) == Abbozza.AUTHORS_MODE ) {
            Abbozza.setEditMode ( Number(editMode) );
        }
    }
...
Configuration.setParameter = function (par, value) {
    ...
    switch (par) {
        ...
        case "serverPort" :
            ...
            break;
        case "tasksEditable" :
            console.log("Deprecated Option taskEditable. Replaced by editMode!");
            break;
            /*
            if ( Configuration.getParameter("tasksEditable") == null ) {
                TaskWindow.setEditable(true);
                Abbozza.tasksEditable = true;
            } else {
                var flag = (Configuration.getParameter("tasksEditable") == "true");
                TaskWindow.setEditable(flag);
                Abbozza.tasksEditable = flag;
            }
                    */
        case "mode" :
            Abbozza.setEditMode( Number(value) );
            if ( Abbozza.editMode == Abbozza.WORKSHOP_MODE ) {
                Abbozza.tasksEditable = false;
                TaskWindow.setEditable(false);
            } else {
                TaskWindow.setEditable(true);
                Abbozza.tasksEditable = true;                
            }
...
/**
 * Returns a list of variable blocks, accorindg to the current configuration.
 * 
 * @returns {Array} An array containing the block ids.
 */
Configuration.getVariableBlocks = function() {
    var list;
    if (Configuration.getParameter("option.noArrays") == "true") {
        list = ['var_num_noconn', 'var_string_noconn', 'var_decimal_noconn', 'var_boolean_noconn'];
    } else if (Configuration.getParameter("option.linArrays") == "true") {
        list = ['var_num', 'var_string', 'var_decimal', 'var_boolean', 'arr_dimension_noconn'];
    } else {
        list = ['var_num', 'var_string', 'var_decimal', 'var_boolean', 'arr_dimension'];
    }
    if (Configuration.getParameter("option.stack") == "true") {
        list.push('var_stack');
    }
    if (Configuration.getParameter("option.queue") == "true") {
        list.push('var_queue');
    }
    if (Configuration.getParameter("option.list") == "true") {
        list.push('var_list');
    }
    if (Configuration.getParameter("option.bintree") == "true") {
        list.push('var_bintree');
    }
    
    // Add variable blocks for the system
    if ( Abbozza.addVariableBlocks) {
        Abbozza.addVariableBlocks(list);
    }
    return list;
}
