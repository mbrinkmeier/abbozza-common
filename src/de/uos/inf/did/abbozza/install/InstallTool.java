...
public abstract class InstallTool {
    ...
    /**
     * This operation checks if the user has administrative rights. Only then
     * the global installation is allowed.
     *
    ...
    public abstract String checkBrowsers();

    public String checkBrowsers(String paths[]) {            
        AbbozzaLogger.info("Looking for standard browsers");
        for (int i = 0; i < paths.length; i++) {
            AbbozzaLogger.info("... Checking path " + paths[i]);
            if (Files.isExecutable(new File(paths[i]).toPath())) {
                AbbozzaLogger.info("... Found browser at " + paths[i]);
                return paths[i];
            }
        }
        return null;
